...
import { media, postSearch, setShowShare } from 'flux/modules/post/actions';
...
const PostSearch = () => {
  ...
  const { data: dataMedia, status: statusMedia } = useMedia();
  const { data: posts, status: statusPost } = usePostSearch();
  ...
  const isLoading =
    ...
    statusPost === RequestStatus.fetching;
  ...
  useEffect(() => {
    if (posts) {
      if (posts.length && !isFullMedia) {
        forEach(posts, post => dispatch(media.request(post.featured_media)));
      }
    }
  }, [posts]);
    ...
    <S.Wrapper>
      <S.HeaderWrapper>
        <S.Header>
          ...
          <HeaderDesktopPost />
          <S.TitleWrapper>
            <S.Title>Você buscou por:</S.Title>
            <h2>{decodeURIComponent(search)}</h2>
          </S.TitleWrapper>
        </S.Header>
      </S.HeaderWrapper>
      <S.Content>
        <S.Text>Resultados</S.Text>
        <S.CardWrapper>
